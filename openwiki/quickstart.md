...
`media-tools` is a Python package that exposes **49+ multimedia processing tools** via two interfaces:
...
|----------|-----------|--------------|-----------------|
| [PDF](/openwiki/tools/pdf.md) | 23 | 18 | pypdf, pdfplumber, pypdfium2, liteparse, pdf2docx, Ghostscript, firecrawl |
| [Image](/openwiki/tools/image.md) | 7 | 12 | Pillow, pillow-heif, pytesseract, rembg |
| [Audio](/openwiki/tools/audio.md) | 6 | 5 | pydub |
| [Video](/openwiki/tools/video.md) | 7 | 4 | ffmpeg, moviepy, opencv |
| [Office](/openwiki/tools/office.md) | 2 | 2 | markitdown, LibreOffice |
| [AI Documents](/openwiki/tools/ai.md) | 3 | 0 | anydoc, LiteLLM (Gemma 4) |
| [Text-to-Speech](/openwiki/tools/tts.md) | 1 | 0 | LiteLLM proxy (Kokoro) |

The 49 MCP tools are tagged (`pdf`, `image`, `audio`, `video`, `office`, `ai`, `tts`) so category-scoped servers can expose a single category.
...
- **Server**: `src/media_tools/server.py` — FastMCP server with 49 `@mcp.tool` registrations, each tagged (`pdf`, `image`, `audio`, `video`, `office`, `ai`, `tts`).
- **CLI**: `src/media_tools/cli.py` — argparse-based dispatcher with 42 handlers across 5 categories (pdf, image, audio, video, office).
- **Toolkits**: `src/media_tools/tools/{pdf,image,audio,video,office,ai,tts}.py` — pure static-method classes.
- **Utilities**: `src/media_tools/utils.py` — Shared validation, logging, subprocess helpers.
- **Console scripts**: 8 in `pyproject.toml` — `media-tools` (CLI) + `media-tools-server` (all tools) + 7 category-scoped servers (`media-tools-server-{pdf,image,audio,video,office,ai,tts}`) that serve only tools with the matching tag.
...
The following areas were identified but deferred from this documentation pass:

| Area | Source Anchor | Reason Deferred |
|------|---------------|-----------------|
| `convert_pdf.py` standalone script | `/scripts/convert_pdf.py` | One-off DeepSeek OCR pipeline with LaTeX cleanup; not part of the main package API |
...
| LiteParse structured extraction internals | `/src/media_tools/tools/pdf.py` | Optional dependency; low usage frequency |
| Unexposed toolkit methods | `src/media_tools/tools/{pdf,image,audio,video}.py` | `PDFToolkit.ocr`/`crop`, `ImageToolkit.watermark`/`collage`, `AudioToolkit.normalize`, and `VideoToolkit.crop`/`rotate`/`resize`/`reverse`/`speed`/`merge`/`watermark`/`subtitle_burn` — implemented but not wired to MCP/CLI |
