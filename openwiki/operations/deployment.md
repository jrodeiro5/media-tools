...
`pyproject.toml` defines eight console scripts. The first (`media-tools-server`) exposes all 49 tools; the remaining seven are category-scoped and run `mcp.enable(tags={tag}, only=True)` so only tools with that tag are served:
...
This lets an operator run a lean server exposing a single category instead of all 49 tools.
