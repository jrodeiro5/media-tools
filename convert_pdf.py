...
def run_deepseek_ocr(input_path: str, output_path: str, api_key: str):
    print("\n--- Running DeepSeek V4 Vision OCR ---")
    start = time.time()
    try:
        import base64
        ...
        for idx, page_indices in enumerate(chunks):
            ...
            prompt = (
                f"Convert pages {page_indices[0]+1} to {page_indices[-1]+1} of this document into clean, well-formatted Markdown. "
                "Format tables as markdown tables, mathematical expressions as LaTeX ($...$ or $$...$$), preserve section headers. "
        ...
        print(f"\n✅ Success! DeepSeek OCR completed in {time.time() - start:.2f}s.")
        print(f"Output saved to: {output_path}")

    except Exception as e:
        print(f"❌ Error during DeepSeek OCR: {e}")
